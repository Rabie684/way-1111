         ...
         <div className="flex h-screen md:p-4 md:gap-4">
            ...
            <aside className={`fixed top-0 ltr:left-0 rtl:right-0 h-full w-3/4 sm:w-1/2 md:w-1/4 bg-white dark:bg-gray-800 border-e dark:border-gray-700 flex flex-col transform transition-transform duration-300 ease-in-out z-30 md:relative md:translate-x-0 md:h-full md:rounded-xl md:border-0 md:shadow-lg ${isSidebarOpen ? 'ltr:translate-x-0 rtl:-translate-x-0' : 'ltr:-translate-x-full rtl:translate-x-full'}`}>
                ...
                <nav className="p-4 space-y-1">
                    <button onClick={() => handleSelectTab('my-channels')} className={`w-full flex items-center p-2 rounded-md text-sm font-medium ${activeTab === 'my-channels' ? 'bg-primary-100 dark:bg-primary-900/50 text-primary-700 dark:text-primary-300' : 'text-gray-900 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                        <BookOpenIcon className="w-5 h-5 me-3"/> <span>{s.myChannels}</span>
                    </button>
                ...
                {activeTab === 'my-channels' && (
                     <div className="flex-1 flex flex-col border-t border-gray-200 dark:border-gray-700 overflow-y-hidden">
                        <nav className="flex-1 overflow-y-auto p-2">
                            ...
                            {channels.filter(ch => user.subscribedChannels.includes(ch.id)).length === 0 && (
