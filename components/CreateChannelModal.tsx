...
const CreateChannelModal: React.FC<CreateChannelModalProps> = ({ onClose }) => {
    const { createChannel, language } = useApp();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if(name && specialization) {
            createChannel({ name, specialization, meetLink: 'https://meet.google.com/new' });
            onClose();
        }
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md m-4">
                <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                    <h2 className="text-xl font-bold">{s.createChannel}</h2>
                </div>
